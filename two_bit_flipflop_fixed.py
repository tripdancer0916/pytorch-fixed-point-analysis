...
class TwoBitFlipFlopMixed(object):
    def __init__(self, time_length, u1_mean=10, u2_mean=10):
        self.time_length = time_length
        self.u1_mean = u1_mean
        self.u2_mean = u2_mean
    ...
    def getitem(self):
        ...
        for index in range(self.time_length):
            z1_target[index] = u1_signal_record[index]
            z2_target[index] = u2_signal_record[index] * u2_signal_record[index]
...
def main(config_path):
    ...
    if 'Mixed' in cfg['MODEL']['NAME']:
        eval_dataset = TwoBitFlipFlop(time_length=cfg['DATALOADER']['TIME_LENGTH'],
                                      u1_mean=cfg['DATALOADER']['U1_MEAN'],
                                      u2_mean=cfg['DATALOADER']['U2_MEAN'])
    else:
        eval_dataset = TwoBitFlipFlopMixed(time_length=cfg['DATALOADER']['TIME_LENGTH'],
                                           u1_mean=cfg['DATALOADER']['U1_MEAN'],
                                           u2_mean=cfg['DATALOADER']['U2_MEAN'])
